...
def load_sequence_data(keypoints_dir, labels_df):
    """
    Load keypoints data as sequences (not aggregated) for LSTM processing.
...
class LSTMCell:
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.W_f = np.random.normal(0, scale, (input_size, hidden_size))  # Forget gate
        self.W_i = np.random.normal(0, scale, (input_size, hidden_size))  # Input gate
        self.W_c = np.random.normal(0, scale, (input_size, hidden_size))  # Candidate values
        self.W_o = np.random.normal(0, scale, (input_size, hidden_size))  # Output gate
        
        scale_h = np.sqrt(1.0 / hidden_size)
        self.U_f = np.random.normal(0, scale_h, (hidden_size, hidden_size))  # Forget gate
        self.U_i = np.random.normal(0, scale_h, (hidden_size, hidden_size))  # Input gate
        self.U_c = np.random.normal(0, scale_h, (hidden_size, hidden_size))  # Candidate values
        self.U_o = np.random.normal(0, scale_h, (hidden_size, hidden_size))  # Output gate
    ...
    def forward(self, x, h_prev, c_prev):
        ...
        # Candidate values
        c_tilde = self.tanh(x @ self.W_c + h_prev @ self.U_c + self.b_c)
        ...
        cache = {
            ...
            'f': f, 'i': i, 'c_tilde': c_tilde, 'c': c, 'o': o, 'h': h
        }
    ...
    def backward(self, dh, dc, cache):
        ...
        # Gradient through tanh in h = o * tanh(c)
        dtanh_c = self.tanh(c)
        do = dh * dtanh_c
        dc_from_h = dh * o * (1 - dtanh_c ** 2)
        dc_total = dc + dc_from_h
        
        # Gradient through cell state update: c = f * c_prev + i * c_tilde
        dc_prev = dc_total * f
        df = dc_total * c_prev
        di = dc_total * c_tilde
        dc_tilde = dc_total * i
        ...
        dx = (do_raw @ self.W_o.T + di_raw @ self.W_i.T + 
              df_raw @ self.W_f.T + dc_tilde_raw @ self.W_c.T)
        
        dh_prev = (do_raw @ self.U_o.T + di_raw @ self.U_i.T + 
                   df_raw @ self.U_f.T + dc_tilde_raw @ self.U_c.T)
        
        # Parameter gradients (average over batch for mini-batch gradient descent)
        batch_size = x.shape[0]
        grads = {
            'W_f': (x.T @ df_raw) / batch_size, 
            'U_f': (h_prev.T @ df_raw) / batch_size, 
            'b_f': np.sum(df_raw, axis=0) / batch_size,
            'W_i': (x.T @ di_raw) / batch_size, 
            'U_i': (h_prev.T @ di_raw) / batch_size, 
            'b_i': np.sum(di_raw, axis=0) / batch_size,
            'W_c': (x.T @ dc_tilde_raw) / batch_size, 
            'U_c': (h_prev.T @ dc_tilde_raw) / batch_size, 
            'b_c': np.sum(dc_tilde_raw, axis=0) / batch_size,
            'W_o': (x.T @ do_raw) / batch_size, 
            'U_o': (h_prev.T @ do_raw) / batch_size, 
            'b_o': np.sum(do_raw, axis=0) / batch_size,
        }
...
class LSTMClassifier:
    ...
    def forward(self, X, sequence_lengths):
        ...
        for t in range(max_length):
            ...
            for layer_idx, lstm_cell in enumerate(self.lstm_cells):
                ...
                h, c, cache = lstm_cell.forward(x_t, h_prev, c_prev)
                ...
                c_states[layer_idx] = c * valid_mask + c_states[layer_idx] * (1 - valid_mask)
        ...
        return probs, {'caches': caches, 'final_h': final_h, 'logits': logits, 
                      'h_states': h_states, 'c_states': c_states, 'max_length': max_length}
    
    def backward(self, probs, y_onehot, cache, sequence_lengths):
        """
        Backward pass through LSTM using Backpropagation Through Time (BPTT).
        ...
        # Initialize gradients for LSTM cells
        grads = {
            'W_out': dW_out,
            'b_out': db_out
        }
        ...
        for layer_idx in range(self.num_layers):
            ...
            grads[f'W_f_{layer_idx}'] = np.zeros_like(cell.W_f)
            grads[f'U_f_{layer_idx}'] = np.zeros_like(cell.U_f)
            grads[f'b_f_{layer_idx}'] = np.zeros_like(cell.b_f)
            grads[f'W_i_{layer_idx}'] = np.zeros_like(cell.W_i)
            grads[f'U_i_{layer_idx}'] = np.zeros_like(cell.U_i)
            grads[f'b_i_{layer_idx}'] = np.zeros_like(cell.b_i)
            grads[f'W_c_{layer_idx}'] = np.zeros_like(cell.W_c)
            grads[f'U_c_{layer_idx}'] = np.zeros_like(cell.U_c)
            grads[f'b_c_{layer_idx}'] = np.zeros_like(cell.b_c)
            grads[f'W_o_{layer_idx}'] = np.zeros_like(cell.W_o)
            grads[f'U_o_{layer_idx}'] = np.zeros_like(cell.U_o)
            grads[f'b_o_{layer_idx}'] = np.zeros_like(cell.b_o)
        ...
        for layer_idx in reversed(range(self.num_layers)):
            ...
            if layer_idx == self.num_layers - 1:
                # Find the last valid time step for each sequence
                last_valid_t = sequence_lengths - 1
                # Only apply gradient to the final hidden state (which is at last valid time step)
                dh = dh_final.copy()
            else:
            ...
            for t in reversed(range(max_length)):
                ...
                dc_masked = dc * valid_mask
                
                dx, dh_prev, dc_prev, cell_grads = cell.backward(dh_masked, dc_masked, cell_cache)
                
                # Accumulate parameter gradients (only from valid time steps)
                # Note: cell_grads already contain gradients averaged over the batch in backward()
                # But we need to average over time steps too, so we accumulate and will average later
                grads[f'W_f_{layer_idx}'] += cell_grads['W_f']
                grads[f'U_f_{layer_idx}'] += cell_grads['U_f']
                grads[f'b_f_{layer_idx}'] += cell_grads['b_f']
                grads[f'W_i_{layer_idx}'] += cell_grads['W_i']
                grads[f'U_i_{layer_idx}'] += cell_grads['U_i']
                grads[f'b_i_{layer_idx}'] += cell_grads['b_i']
                grads[f'W_c_{layer_idx}'] += cell_grads['W_c']
                grads[f'U_c_{layer_idx}'] += cell_grads['U_c']
                grads[f'b_c_{layer_idx}'] += cell_grads['b_c']
                grads[f'W_o_{layer_idx}'] += cell_grads['W_o']
                grads[f'U_o_{layer_idx}'] += cell_grads['U_o']
                grads[f'b_o_{layer_idx}'] += cell_grads['b_o']
    ...
    def update_params(self, grads, learning_rate, clip_value=5.0):
        ...
        for layer_idx in range(self.num_layers):
            cell = self.lstm_cells[layer_idx]
            cell.W_f -= learning_rate * grads[f'W_f_{layer_idx}']
            cell.U_f -= learning_rate * grads[f'U_f_{layer_idx}']
            cell.b_f -= learning_rate * grads[f'b_f_{layer_idx}']
            cell.W_i -= learning_rate * grads[f'W_i_{layer_idx}']
            cell.U_i -= learning_rate * grads[f'U_i_{layer_idx}']
            cell.b_i -= learning_rate * grads[f'b_i_{layer_idx}']
            cell.W_c -= learning_rate * grads[f'W_c_{layer_idx}']
            cell.U_c -= learning_rate * grads[f'U_c_{layer_idx}']
            cell.b_c -= learning_rate * grads[f'b_c_{layer_idx}']
            cell.W_o -= learning_rate * grads[f'W_o_{layer_idx}']
            cell.U_o -= learning_rate * grads[f'U_o_{layer_idx}']
            cell.b_o -= learning_rate * grads[f'b_o_{layer_idx}']
...
def train_lstm(model, X_train, y_train, X_dev, y_dev, sequence_lengths_train, 
               sequence_lengths_dev, learning_rate=0.001, num_epochs=50, 
...
def main():
    """
    Main function to train and evaluate LSTM model for tennis serve classification.
    ...
    output_dir = project_dir / 'src' / 'lstm' / 'test_results'
    ...
    # Train model
    train_losses, dev_losses, train_accuracies, dev_accuracies = train_lstm(
    ...
    if len(X_test_padded) > 0:
        ...
        test_pred_path = output_dir / f'lstm_test_predictions_seed_{seed}.txt'
        ...
        # Plot confusion matrix
        cm_path = output_dir / f'lstm_confusion_matrix_seed_{seed}.png'
        plot_confusion_matrix(y_test, test_preds, direction_classes, str(cm_path))
        
        # Save accuracy summary
        summary_path = output_dir / f'lstm_accuracy_summary_seed_{seed}.txt'
